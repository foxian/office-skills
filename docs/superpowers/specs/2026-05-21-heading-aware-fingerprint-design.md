...
**实现要点：**
- 读取样式 XML：通过 `style.element.find(qn('w:pPr'))` 再 `.find(qn('w:outlineLvl'))` 获取（与现有代码 `_detect_list_type` 的 `pPr.find(qn('w:numPr'))` 风格一致；不用点属性访问，避免无节点时 AttributeError）
...
| 字段 | 聚合方式 | None 处理 |
|------|---------|----------|
| `size` | 数值平均，统一格式化为 `f"{val:.1f}pt"`（1位小数，单段时同样适用） | 跳过 None 段，只平均有值的；若全为 None 则结果为 None |
| `bold` | 多数投票（True / False / None 各计数，取最多的）。**平局规则**：按 False > None > True 优先级取胜（偏保守，避免误加粗） | None 作为独立选项参与投票；若 None 票最多，结果为 None（保留"继承自样式"语义） |
| `italic` | 同 bold，平局规则同（False > None > True） | 同 bold |
| `align` | 多数投票。**平局规则**：取该级别第一个段落的 align 值 | None 作为独立选项参与投票 |
| `font` | 出现频率最高的字符串值。**平局规则**：取该级别第一个段落的 font 值 | None 跳过，不参与投票；若全为 None 则结果为 None |
| `color` | 出现频率最高的字符串值。**平局规则**：取该级别第一个段落的 color 值 | None 作为独立选项参与投票 *（None 代表"跟随主题色"，具有独立语义，故参与投票，与 font 的跳过行为不同）* |
| `space_before` | 数值平均，格式化为 `f"{val:.1f}pt"` | 视为 0.0pt（与 compute_effective_fingerprint 行为一致） |
| `space_after` | 同 space_before | 同上 |
| `first_line_indent` | 同 space_before | 同上 |
| `line_spacing` | 数值平均（仅 float 类型） | 跳过 None；若全为 None 则结果为 None |

**example 字段选取规则：** 取该大纲级别段落列表中**第一个段落**的文本（按文档中出现顺序）。
...
[
  {
    ...
    "fingerprint": {"size": "16.0pt", "bold": true, "italic": false, "align": "center", "font": "黑体", "color": null, "space_before": "12.0pt", "space_after": "6.0pt", "first_line_indent": "0.0pt", "line_spacing": null},
    "example": "第一章 概述"
  },
  {
    "heading_role": "Heading 3",
    "fingerprint": {"size": "13.0pt", "bold": true, "italic": false, "align": "justify", "font": "黑体", "color": null, "space_before": "0.0pt", "space_after": "0.0pt", "first_line_indent": "0.0pt", "line_spacing": null},
    "example": "三级标题示例（此段实际用了Heading 4样式，但outlineLvl=2）"
  },
  {
    "id": 0,
    "fingerprint": {"size": "12.0pt", "bold": false, "italic": false, "align": "justify", "font": "宋体", "color": null, "space_before": "0.0pt", "space_after": "0.0pt", "first_line_indent": "24.0pt", "line_spacing": 1.5},
    "example": "正文内容"
  }
]
```
...
5. **单元测试**：同一大纲级别多段，`size` 字段取数值平均，格式化1位小数；`size=None` 的段跳过不参与平均
6. **单元测试**：`bold` 平局时按 False > None > True 优先级取胜（如 True:1 False:1 → 结果 False；True:1 False:1 None:1 → 结果 False）
7. **单元测试**：`bold` None 票最多时结果为 None（如 None:2 True:1 → 结果 None）
8. **集成测试**：文档含样式名正确标题（Heading 3 样式 + outlineLvl=2）+ 样式名错误标题（Heading 4 样式 + outlineLvl=2）+ 正文，heading-aware 模式输出：两段均归入同一 Heading 3 分组，正文走聚类逻辑
9. **回归测试**：`heading_aware=False`（默认）下，所有原有测试全部通过
